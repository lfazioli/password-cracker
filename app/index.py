...
def hash_password(password, algorithm='sha256'):
    """Genera un hash della password con l'algoritmo richiesto."""
    password_bytes = password.encode()

    if algorithm == 'md5':
        pass_code = hashlib.md5(password_bytes).hexdigest()
        return pass_code
    elif algorithm == 'sha1':
        pass_code = hashlib.sha1(password_bytes).hexdigest()
        return pass_code
    elif algorithm == 'sha256':
        pass_code = hashlib.sha256(password_bytes).hexdigest()
        return pass_code
    elif algorithm == 'sha512':
        pass_code = hashlib.sha512(password_bytes).hexdigest()
        return pass_code
    elif algorithm == 'blake2b':
        pass_code = hashlib.blake2b(password_bytes).hexdigest()
        return pass_code
    elif algorithm == 'blake2s':
        pass_code = hashlib.blake2s(password_bytes).hexdigest()
        return pass_code
    else:
...
def extract_passwords_from_files(files):
    """Estrae password dai file uploadati."""
    all_passwords = []
...
def save_stats(stats):
    """Salva le statistiche di cracking in un file JSON."""
    filename = "stats_log.json"
    try:
...
def generate(target_password, passwords, hash_algorithm):
    """Genera un flusso di eventi per il cracking delle password."""
    total = len(passwords)
...
def crack():
    """Gestisce la richiesta di cracking della password."""
...
def download_json():
    """Scarica le statistiche in formato JSON."""
    path = 'stats_log.json'
...
def download_csv():
    """Scarica le statistiche in formato CSV."""
    json_path = 'stats_log.json'
